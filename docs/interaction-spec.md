...
### 15.0 APP 首页球体模式（已实现范围）

React 训练首页在首屏嵌入共享的 `planet.html?mode=app` 场景，使用完整连续地形、缩放细节、气候动画和区域拾取，不再显示 CSS 装饰球。APP 模式隐藏“探索尺度”和页面底部的操作说明/统计行；保留自动旋转、区域快捷入口、区域指标卡片、放大、缩小和复位。切换到其他主导航或进入训练记录时卸载场景，避免隐藏页面长期占用 GPU。

身体指标说明随区域卡片展示：造山带对应骨骼肌量起伏，深蓝寰海对应体内水分状态，极地冰原对应骨量，季风林海至赤砂荒原对应体脂率地貌映射。“天脉长河”的动画表达血液循环的视觉意象，不提供实际血流测量。当前数值继续标为模拟数据；后续接入身体数据沿用稳定区域 ID。

### 15.1 地图式缩放层级（2026-09-22）

星球独立预览采用五级信息递进：天际 / 板块 / 气候 / 生态 / 地表。滚轮、双指与缩放按钮连续改变观察距离；左侧显示 0–100% 缩放进度并可直接到达对应层级。主要内容在前 80% 缩放过程中依次出现，最后 20% 用于岩层、灌木、冰川裂隙和独立地标。进入更深层级时保持当前观察方向，底部区域入口仍可旋转到指定身体区域。

- 天际：星球轮廓、极地与稀疏薄云；山体使用约 26% 的远景起伏，隐藏地名和微观物件。
- 板块：显示大陆、海洋、冰川和群岛名称；保持简化的地形轮廓。
- 气候：展开雨云、气流、主河道和气候相关地貌名称。
- 生态：恢复主要山体起伏，展开成片针叶林、支流、湖泊和基础水流。
- 地表：展开灌木、岩石、冰川裂隙、独立地标及岩面细节。

内容按连续权重渐入/渐出，标签也按缩放百分比过渡；层级标题使用滞回阈值减少边界闪动。缩回时依相反顺序收起。地标标签避让卡片、控制区和其他标签。近景局部细分跟随当前观察方向，无需先选择身体区域。缩放不改变身体区域 ID 或指标值。


### 15.2 桌面鼠标旋转手势

鼠标悬停不会改变星球朝向。左键短按用于点选地貌；按住约 180ms 后进入旋转状态，保持按下并移动才会转动。按下后先有轻微移动也不会取消长按，达到时长后继续拖动即可旋转。地表移动方向与指针拖动方向一致，四元数插值吸收指针事件的速度波动。星球可穿过冰川极区并从任意方向继续转动；方向键遵循同一旋转方式。单指触控可直接旋转，双指操作用于缩放。
