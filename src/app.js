...
renderer.setPixelRatio(Math.min(devicePixelRatio,1.6));renderer.outputColorSpace=T.SRGBColorSpace;host.appendChild(renderer.domElement);
...
const ramp=new T.DataTexture(new Uint8Array([72,118,174,230]),4,1,T.RedFormat);ramp.needsUpdate=true;ramp.minFilter=ramp.magFilter=T.NearestFilter;
...
function patch(lat,lon,rx,ry,color,zone,height=.045,phase=0){const N=64,R=8,verts=[],ids=[];verts.push(...point(lat,lon,1+height).toArray());for(let j=1;j<=R;j++)for(let i=0;i<N;i++){let a=i/N*Math.PI*2,f=j/R,w=1+.10*Math.sin(5*a+phase)+.055*Math.cos(9*a-phase);verts.push(...point(lat+Math.sin(a)*ry*f*w,lon+Math.cos(a)*rx*f*w,1+height).toArray());}for(let i=0;i<N;i++)ids.push(0,1+i,1+(i+1)%N);for(let j=1;j<R;j++)for(let i=0;i<N;i++){let a=1+(j-1)*N+i,b=1+(j-1)*N+(i+1)%N,c=a+N,d=b+N;ids.push(a,c,b,b,c,d);}const start=verts.length/3;for(let i=0;i<N;i++){let a=i/N*Math.PI*2,w=1+.10*Math.sin(5*a+phase)+.055*Math.cos(9*a-phase);verts.push(...point(lat+Math.sin(a)*ry*w,lon+Math.cos(a)*rx*w,1.003).toArray());let t=1+(R-1)*N+i,u=1+(R-1)*N+(i+1)%N;ids.push(t,start+i,u,u,start+i,start+(i+1)%N);}const geo=new T.BufferGeometry();geo.setAttribute('position',new T.Float32BufferAttribute(verts,3));geo.setIndex(ids);geo.computeVertexNormals(); // Geographic parameterization has outward clockwise rings.
const pos=geo.attributes.position;const index=geo.index;const a=new T.Vector3().fromBufferAttribute(pos,index.getX(0)),b=new T.Vector3().fromBufferAttribute(pos,index.getX(1)),c=new T.Vector3().fromBufferAttribute(pos,index.getX(2));if(b.clone().sub(a).cross(c.clone().sub(a)).dot(a)<0){for(let i=0;i<index.count;i+=3){const t=index.getX(i+1);index.setX(i+1,index.getX(i+2));index.setX(i+2,t);}geo.computeVertexNormals();}return mesh(geo,color,root,zone);}
...
const mountainBases=[];function mountain(lat,lon,h,w){const g=new T.Group();root.add(g);g.position.copy(point(lat,lon,1.07));g.quaternion.setFromUnitVectors(new T.Vector3(0,1,0),point(lat,lon));const rock=mesh(new T.ConeGeometry(w,h,7,1),0xa66d50,g,'muscle',true);rock.position.y=h*.40;rock.rotation.y=rand()*3;const snow=mesh(new T.ConeGeometry(w*.31,h*.31,7),0xd9dcd7,g,'muscle',true);snow.position.y=h*.745;snow.rotation.y=rock.rotation.y;mountainBases.push(g);}
...
function river(coords,width,color=0x63c9c6,zone='water',r=1.083){const curve=new T.CatmullRomCurve3(coords.map(p=>point(p[0],p[1],r)));return mesh(new T.TubeGeometry(curve,70,width,5,false),color,root,zone);}
...
const trees=[];for(let i=0;i<155;i++){let lat=-35+rand()*34,lon=7+rand()*45;if(((lat+18)/22)**2+((lon-27)/30)**2>1||Math.abs(lon-(22-lat*.48))<5)continue;const s=.018+rand()*.02;const m=place(new T.ConeGeometry(s,s*2.5,5),i%3?0x3e6255:0x557761,lat,lon,1.065+s,'fat');trees.push(m);if(i%3!==0)details.push(m);}
...
let last=performance.now(),detail=0;function frame(now){requestAnimationFrame(frame);const dt=Math.min((now-last)/1000,.06);last=now;if(document.hidden)return;if(targetCamera){camera.position.lerp(targetCamera,reduced?1:1-Math.exp(-dt*5));if(camera.position.distanceTo(targetCamera)<.005)targetCamera=null;}controls.update(dt);root.updateMatrixWorld();const distance=camera.position.length();const desired=1-T.MathUtils.smoothstep(distance,2.55,3.35);detail+=(desired-detail)*Math.min(1,dt*6);details.forEach(m=>{m.visible=detail>.025;m.scale.setScalar(Math.max(.001,detail));});$('#level').textContent=distance<2.55?'山川近景':distance<3.4?'大陆视角':'星球全貌';$('#detail-label').textContent=distance<3.1?'支流与植被 · 细节已展开':'远观轮廓 · 近看山川';updateLandmarks(distance<3.1?1:0);renderer.render(scene,camera);}requestAnimationFrame(frame);$('#loading').hidden=true;
